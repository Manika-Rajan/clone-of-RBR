...
const Payment = () => {
  ...
  const { reportId, amount } = location.state || {};
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const response = await fetch('https://d7vdzrifz9.execute-api.ap-south-1.amazonaws.com/prod/create-razorpay-order', {
        ...
        body: JSON.stringify({ reportId, amount: amount * 100, userId }),
      });
        ...
        handler: async (response) => {
          try {
            ...
            if (verifyResponse.ok) {
              // Log successful payment to DynamoDB with both IDs
              ...
              await fetch('https://your-api-gateway-id.execute-api.ap-south-1.amazonaws.com/prod/log-payment', {
                ...
                body: JSON.stringify({
                  reportId,
                  file_key: location.state.file_key, // Include file_key
                  userId,
                  status: 'success',
                  amount: amount * 100,
      ...
      rzp.on('payment.failed', async (response) => {
        ...
        // Log failed payment to DynamoDB with both IDs
        const now = new Date().toISOString();
        await fetch('https://your-api-gateway-id.execute-api.ap-south-1.amazonaws.com/prod/log-payment', {
          method: 'POST',
          headers: {
